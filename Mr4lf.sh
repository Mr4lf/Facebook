...
pu='\033[37;1m' #putih
ku='\033[33;1m' #kuning
# Mukanya Kwntl Yg liat Ini hwhwhwh :v
echo
echo
echo
...
echo $i"|"$me" 2"$i" |"$cy" BRUTEFORCE FB Versi 2    "$i"|"$i"    |"$cy" E"$i" |"
echo $i"|"$me" 3"$i" |"$cy" BRUTEFORCE FB Versi 3    "$i"|"$i"    |"$cy" N"$i" |"
echo $i"|"$me" 4"$i" |"$cy" BRUTEFORCE FB MBF        "$i"|"$i"    |"$cy" G"$i" |"
echo $i"|"$me" 5"$i" |"$cy" FACEBRUTE FB Mwmwk       "$i"|"$i"    |"$cy" K"$i" |"
...
echo $i"────────────────────────────────"$i"    |"$cy" R"$i" |"
echo $i"|"$me"18"$i" |"$cy" INSTALL Bokepnya Dulu:v  "$i"|"$i"    |"$cy" E"$i" |"
echo $i"|"$me"19"$i" |"$cy" KELUAR/EXIT PROGRAM      "$i"|"$i"    |"$cy" N"$i" |"
echo $i"────────────────────────────────    ─────"
echo
echo $me"┌==="$bi"["$i"Mr4lf"$bi"]"$me"======"$bi"["$i""Pilih Nomornya Bangsad""$bi"]"
echo $me"¦"
read -p"└──# " pil
...
pkg install nano
figlet -f slant "Done Y Kwntl "|lolcat
fi
...
sleep 2
echo $cy"Udah Keluar Bangsad Jgn Lama2,soalnya Lu Bau :v"
sleep 2
echo $i"Btw Gua Gans :v Mau Bacot Kontak Gua :)"
sleep 2
echo $ku"WhatsApp :"$i" 083815983767"
echo $ku"Facebook :"$i"====Mr4lf===="
sleep 2
echo $pur">> GK Usah Bilang Makasih Y Gans,Btw Lu Burik,tq..hwhwhwhwhw :v <<"
exit
fi